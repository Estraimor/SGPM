...
<?php endif; ?>
	...
	<div class="contenido">
    <h1>Gestión de Actas Volantes</h1>
    <form method="POST" action="crear_acta_volante.php">
        <div class="cuadro" id="cuadroCarreras">
            <label for="carrera">Seleccione una Carrera:</label>
            <select name="carrera" id="carrera" onchange="cargarMaterias(this.value)">
...
<script>
 function cargarMaterias(idCarrera) {
    ...
    xhr.open('GET', 'obtener_materias.php?idCarrera=' + idCarrera, true);
    xhr.onreadystatechange = function() {
        if (xhr.readyState === 4 && xhr.status === 200) {
            document.getElementById('materias').innerHTML = xhr.responseText;

            // Añadir evento de cambio a cada checkbox para asegurarse de que solo se pueda seleccionar uno
            var checkboxes = document.querySelectorAll('#materias input[type="checkbox"]');
            checkboxes.forEach(function(checkbox) {
                checkbox.addEventListener('change', function() {
                    if (this.checked) {
                        ...
                        obtenerDetallesAlumnos(idCarrera, this.value);
                    } else {
                        // Ocultar el cuadro de información si se desmarca
...
function obtenerDetallesAlumnos(idCarrera, idMateria) {
    var xhr = new XMLHttpRequest();
    xhr.open('GET', 'obtener_cantidad_alumnos_inscriptos.php?idCarrera=' + idCarrera + '&idMateria=' + idMateria, true);
    xhr.onreadystatechange = function() {
        if (xhr.readyState === 4 && xhr.status === 200) {
