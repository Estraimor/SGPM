...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['materia']) && isset($_POST['llamado']) && isset($_POST['tanda']) && isset($_POST['anio'])) {
    ...
    $query_materia = "
    SELECT m.Nombre as materia, p.nombre_profe, p.apellido_profe, c.nombre_carrera as carrera, c.idCarrera,idMaterias 
    FROM materias m
...
$mesActual = date('n'); // 'n' devuelve el mes sin ceros iniciales (1-12)
 ...
 // Determina el texto del encabezado según el mes
    ...
    if ($mesActual == 11 || $mesActual == 12) {
        $encabezado = "MESA EXAMEN FINAL NOVIEMBRE - DICIEMBRE - $anio - ESTUDIANTES REGULARES";
    } elseif ($mesActual == 2 || $mesActual == 3) {
        $encabezado = "MESA EXAMEN FINAL FEBRERO - MARZO - $anio - ESTUDIANTES REGULARES";
    } elseif ($mesActual == 7 || $mesActual == 8) {
        $encabezado = "MESA EXAMEN FINAL JULIO - AGOSTO - $anio - ESTUDIANTES REGULARES";
    } else {
        $encabezado = "MESA EXAMEN FINAL - FECHA NO DISPONIBLE - ESTUDIANTES REGULARES";
    }
        ...
        // Array de IDs de carrera que no deben coincidir
        $idsCarrera = [307,506,507,508,509,510,511,523]; // Agrega aquí todos los IDs de carrera que quieres excluir
