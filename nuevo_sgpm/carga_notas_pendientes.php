...
<label>Buscar por Nombre, Apellido o Legajo:</label>
<input type="text" id="buscarAlumno" placeholder="Ingrese nombre, apellido o legajo">
<br>

<!-- Búsqueda por DNI -->
<label>Buscar por DNI:</label>
<input type="text" id="buscarDNI" placeholder="Ingrese DNI" onkeypress="return event.charCode >= 48 && event.charCode <= 57">
<br>

<!-- Lista de Estudiantes -->
<label>Lista de Estudiantes:</label>
<select id="listaAlumnos">
    <option hidden>Seleccione un estudiante</option>
</select>
    ...
    <div id="modal" style="display: none;">
        <h3>Detalles de Nota</h3>
        <input type="hidden" id="materiaSeleccionada">
        ...
        <br>
        <div id="datosPromocion" style="display: none;">
            <label>Nota Final:</label>
            <input type="number" step="0.01" id="notaFinal">
        </div>
        <div id="datosRegular" style="display: none;">
            <label>Nota Final:</label>
            <input type="number" step="0.01" id="notaFinalRegular">
            <label>Nota Examen Final:</label>
            <input type="number" step="0.01" id="notaExamenFinal">
        </div>
    ...
    $.post('./config_notas_pendientes/buscar_estudiante.php', {}, function(data) {
        $('#listaAlumnos').html(data);
    });

    // Buscar en tiempo real por Nombre, Apellido o Legajo
    $('#buscarAlumno').on('input', function() {
        let query = $(this).val();
        $.post('./config_notas_pendientes/buscar_estudiante.php', { query: query }, function(data) {
            $('#listaAlumnos').html(data);
        });
    });

    // Buscar en tiempo real por DNI
    $('#buscarDNI').on('input', function() {
        let dni = $(this).val();
        $.post('./config_notas_pendientes/buscar_estudiante.php', { dni: dni }, function(data) {
            $('#listaAlumnos').html(data);
        });
    });
            
            // Cargar datos del alumno seleccionado
            $('#listaAlumnos').on('change', function() {
                let legajo = $(this).val();
                $.post('./config_notas_pendientes/obtener_datos_alumno.php', {legajo: legajo}, function(data) {
                    let alumno = JSON.parse(data);
                    $('#carrera').val(alumno.carrera);
                    $('#comision').html(alumno.comisiones);
                    $('#curso').html(alumno.cursos);
                });
            });
            
            // Cargar materias según curso
            $('#curso').on('change', function() {
                let curso = $(this).val();
                let comision = $('#comision').val();
                let carrera = $('#carrera').val();
                $.post('./config_notas_pendientes/obtener_materias.php', {curso: curso, comision: comision, carrera: carrera}, function(data) {
                    $('#listaMaterias').html(data);
                });
            });
            
            // Mostrar condición al seleccionar materia
            $(document).on('change', '.materiaCheckbox', function() {
                let materia = $(this).val();
                if ($(this).is(':checked')) {
                    $('#materiaSeleccionada').val(materia);
                    $('#modal').show();
                }
            });
            
            // Mostrar campos según condición
            ...
            $('#guardarNota').on('click', function() {
                ...
                let notaFinal = $('#notaFinal').val();
                let notaFinalRegular = $('#notaFinalRegular').val();
                let notaExamenFinal = $('#notaExamenFinal').val();
                
                let datos = {
                    legajo: legajo,
                    materia: materia,
                    condicion: condicion,
                    notaFinal: condicion === 'Promocion' ? notaFinal : notaFinalRegular,
                    notaExamenFinal: notaExamenFinal
                };
                
                $.post('./config_notas_pendientes/guardar_notas.php', datos, function(response) {
                    alert(response);
                    location.reload();
                });
            });
