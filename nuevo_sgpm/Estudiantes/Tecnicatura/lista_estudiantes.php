...
<div class="contenido">
  <?php
    ...
    $rolUsuario = $_SESSION["roles"]; // Asumimos que roles está almacenado en la sesión

    // Consulta SQL que varía según el rol del usuario
    if ($rolUsuario == 1) {
      // Si el rol es 1, mostramos todos los alumnos sin importar su estado
      ...
      // Si el rol no es 1, mostramos solo los alumnos con estado '1'
      $sql1 = "SELECT a.*, c.nombre_carrera
               FROM inscripcion_asignatura ia
               INNER JOIN alumno a ON ia.alumno_legajo = a.legajo
  ...
  <div id="tablaContainerEstudiantes">
    <table id="tabla">
      ...
      <tbody>
        <?php
          // Repetimos la consulta SQL según el rol para mostrar los datos en la tabla
          if ($rolUsuario == 1) {
            $sql1 = "SELECT a.*, c.nombre_carrera
                     FROM inscripcion_asignatura ia
                     INNER JOIN alumno a ON ia.alumno_legajo = a.legajo
          ...
          while ($datos = mysqli_fetch_assoc($query1)) {
            ...
            <tr>
              ...
              <td>
                <a href="./ABM_estudiante/modificar_estudianteT.php?legajo=<?php echo $datos['legajo']; ?>" class="modificar-button"><i class="fas fa-pencil-alt"></i></a>
                <a href="#" onclick="return confirmarBorrado('<?php echo $datos['legajo']; ?>')" class="borrar-button"><i class="fas fa-trash-alt"></i></a>
                <a href="info_alumnoT.php?legajo=<?php echo $datos['legajo']; ?>" class="accion-button"><i class="fas fa-exclamation"></i></a>
              </td>
            </tr>
            <?php
          }
