...
GROUP BY 
    ...
    // Apartado de carreras finalizadas
    echo '<br><h2 style="color:white; text-shadow: 2px 1px 2px black;">Carreras Finalizadas</h2><br>';
    
    // Crear el cuadro donde se mostrará la información
    echo '<div style="border: 2px solid white; padding: 10px; color: white; text-shadow: 2px 1px 2px black;">';

    // Consulta para obtener las carreras finalizadas y sus fechas
    $sql_finalizo_fp = "SELECT c.nombre_carrera, fp.fecha_finalizo 
                        FROM finalizo_FP fp
                        INNER JOIN carreras c ON fp.carreras_idCarrera = c.idCarrera
                        WHERE fp.alumnos_fp_legajo_afp = '$legajo';";
    
    $query_finalizo_fp = mysqli_query($conexion, $sql_finalizo_fp);

    if (!$query_finalizo_fp) {
        throw new Exception("Error al obtener las carreras finalizadas: " . mysqli_error($conexion));
    }

    // Si hay resultados, mostrar las carreras finalizadas
    if (mysqli_num_rows($query_finalizo_fp) > 0) {
        ...
        while ($row_finalizo_fp = mysqli_fetch_assoc($query_finalizo_fp)) {
            echo "<tr>
                    <td>{$row_finalizo_fp['nombre_carrera']}</td>
                    <td>{$row_finalizo_fp['fecha_finalizo']}</td>
                  </tr>";
        }

        echo '</table>';
    } else {
        // Si no hay carreras finalizadas
        echo '<p style="text-align: center;">El alumno no ha finalizado ninguna formación profesional.</p>';
    }

    echo '</div>'; // Cerrar el cuadro
...
<style>/* Estilo para el botón de acción */
.accion-button {
