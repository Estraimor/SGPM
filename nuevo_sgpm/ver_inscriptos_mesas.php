...
<div class="contenido">
    <h1 class="titulo">Contador de Inscriptos a Mesas Finales</h1>
    <label for="fecha" class="label-fecha">Selecciona una fecha:</label>
    <input type="date" id="fecha" name="fecha" class="input-fecha">
    <button id="consultar" class="btn-consultar">Consultar</button>

    <div class="tablas-container">
        <div class="tabla-libre">
            <h2 class="condicion">Condición: Libre</h2>
            <table border="1" class="tabla" id="tabla-libres">
                <thead class="tabla-head">
                    <tr class="tabla-row">
                        <th class="tabla-header">Unidad Curricular</th>
                        <th class="tabla-header">Carrera</th>
                        <th class="tabla-header">Tanda</th>
                        <th class="tabla-header">Llamado</th>
                        <th class="tabla-header">Cantidad de Estudiantes</th>
                    </tr>
                </thead>
        ...
        <div class="tabla-regular">
            <h2 class="condicion">Condición: Regular</h2>
            <table border="1" class="tabla" id="tabla-regulares">
                <thead class="tabla-head">
                    <tr class="tabla-row">
                        <th class="tabla-header">Unidad Curricular</th>
                        <th class="tabla-header">Carrera</th>
   ...
   $(document).ready(function () {
        $('#consultar').click(function () {
            ...
            if (fechaSeleccionada) {
                $.ajax({
                    url: './config_inscriptos_mesas/get_mesas.php',
                    method: 'POST',
                    ...
                    success: function (respuesta) {
                        ...
                        // Limpia las tablas
                        $('#tabla-libres .tabla-body').empty();
                        $('#tabla-regulares .tabla-body').empty();

                        // Llena la tabla de libres
                        if (respuesta.libres && respuesta.libres.length > 0) {
                            respuesta.libres.forEach(function (item) {
                                $('#tabla-libres .tabla-body').append(`
                                    <tr>
                                        <td>${item.materia}</td>
                                        <td>${item.carrera}</td>
                                        <td>${item.tanda}</td>
                            ...
                            $('#tabla-libres .tabla-body').append('<tr><td colspan="5">No hay inscriptos con condición Libre.</td></tr>');
                        }

                        // Llena la tabla de regulares
                        if (respuesta.regulares && respuesta.regulares.length > 0) {
                            respuesta.regulares.forEach(function (item) {
                                $('#tabla-regulares .tabla-body').append(`
                                    <tr>
                                        <td>${item.materia}</td>
                                        <td>${item.carrera}</td>
                                        <td>${item.tanda}</td>
                            ...
                            $('#tabla-regulares .tabla-body').append('<tr><td colspan="5">No hay inscriptos con condición Regular.</td></tr>');
                        }
                    },
                    error: function (xhr, status, error) {
                        alert('Error al consultar los datos: ' + error);
                    }
                });
            } else {
...
/* Centro el contenedor de input y botón */
.contenido {
    display: flex;
